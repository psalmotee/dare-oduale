    ...
    <section aria-labelledby="connect-heading">
      <div className="px-6 sm:px-10 md:px-14 py-12 sm:py-16 md:py-24 mx-auto max-w-6xl grid grid-cols-1 lg:grid-cols-2 items-start gap-10 sm:gap-14 lg:gap-8">
        <div className="flex flex-col gap-4 sm:gap-5">
          <h2
            id="connect-heading"
            className="text-2xl md:text-3xl lg:text-4xl font-bold leading-tight tracking-[-0.03em] text-primary"
          >
            Connect Directly
          </h2>
          <p className="max-w-sm text-base sm:text-lg leading-[1.75] text-foreground/60">
          ...
          <p className="text-xs md:text-sm font-semibold uppercase tracking-[0.24em] text-foreground/40">
            All conversations are confidential
          </p>
        </div>

        <div className="rounded-2xl border border-border bg-card p-6 sm:p-8 md:p-12 space-y-6">
          <h3
            ...
            {InfoItems.map((item) => (
              <div key={item.label} className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <p className="text-[10px] md:text-xs font-semibold uppercase tracking-[0.2em] text-foreground/40">
                    {item.label}
                  </p>
                  {item.lines.map((line) => (
                    <p
                      key={line}
                      className="text-xs md:text-sm leading-relaxed text-foreground/70"
                    >
                      {line}
                    </p>
                  ))}
